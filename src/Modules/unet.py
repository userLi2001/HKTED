...
def get_timestep_embedding(timesteps, embedding_dim):
    if isinstance(timesteps, int):
       ...
       assert len(timesteps.shape) == 1  # 确保timesteps是一维张量
...
class Downsample(torch.nn.Module):
    ...
    def forward(self, x):
        if self.with_conv:
            ...
            x = torch.ops.avg_pool2d(x, kernel_size=2, stride=2)
        return x
...
class UNet(torch.nn.Module):
    ...
    def forward(self, x, t):
        ...
        # timestep embedding
        temb = get_timestep_embedding(t, self.ch).to("cuda")
        temb = self.temb[0](temb)
        temb = nonlinearity(temb)
        temb = self.temb[1](temb) # (bs, 4 * hidden_size)
